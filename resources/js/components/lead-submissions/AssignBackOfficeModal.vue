        ...
        <div class="w-full max-w-lg rounded-xl bg-white shadow-xl border border-gray-200 max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div class="flex items-center gap-3 border-b border-gray-200 px-6 py-4 bg-gray-50">
            ...
            <div class="flex-1">
              ...
              <p class="text-xs text-gray-500 mt-0.5">{{ isBulk ? `${bulkLeadIds.length} submission(s) selected` : 'Assign an executive to process this submission' }}</p>
            </div>
            <button
              type="button"
          ...
          <template v-else>
            <div class="space-y-5 px-6 py-5 overflow-y-auto flex-1 min-h-0">
              ...
              <div v-if="!isBulk && lead" class="rounded-lg border border-gray-200 overflow-hidden">
                ...
                <div class="p-4">
                  <dl class="grid grid-cols-2 gap-x-6 gap-y-3">
                    <div>
                      ...
                      <dd class="mt-0.5 text-sm font-semibold text-gray-900">{{ displayVal(lead?.account_number) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Company Name</dt>
                      <dd class="mt-0.5 text-sm font-semibold text-gray-900" :title="lead?.company_name">{{ displayVal(lead?.company_name) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Category</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ displayVal(lead?.category) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Type</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ displayVal(lead?.type) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Product</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ displayVal(lead?.product) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Submission Type</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ displayVal(lead?.submission_type) }}</dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Status</dt>
                      <dd class="mt-1">
                        <span v-if="lead?.status" class="inline-flex rounded-full px-2 py-0.5 text-xs font-medium" :class="statusBadgeClass(lead?.status)">{{ lead?.status }}</span>
                        <span v-else class="text-sm text-gray-400">—</span>
                      </dd>
                    </div>
                    <div>
                      <dt class="text-xs font-medium text-gray-500">Submitted Date</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ formatDate(lead?.submitted_at) }}</dd>
                    </div>
                    <div v-if="lead?.sales_agent">
                      <dt class="text-xs font-medium text-gray-500">Sales Agent</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ lead.sales_agent }}</dd>
                    </div>
                    <div v-if="lead?.executive">
                      <dt class="text-xs font-medium text-gray-500">Current Executive</dt>
                      <dd class="mt-0.5 text-sm text-gray-900">{{ lead.executive }}</dd>
                    </div>
                  </dl>
                </div>
              </div>
              ...
              <div v-else-if="isBulk" class="rounded-lg border border-brand-primary-muted bg-brand-primary-light p-4 flex items-center gap-3">
                ...
                <div>
                  <p class="text-sm font-semibold text-brand-primary-dark">{{ bulkLeadIds.length }} submission(s) selected</p>
                  <p class="text-xs text-brand-primary-hover mt-0.5">All selected submissions will be assigned to the same executive.</p>
                </div>
              </div>

              <!-- Select Executive -->
              <div>
                <label for="assign-executive" class="block text-sm font-medium text-gray-700 mb-1.5">
