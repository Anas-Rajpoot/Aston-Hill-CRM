...
const SORTABLE_COLUMNS = [
  ...
  'contact_number', 'manager', 'team_leader', 'sales_agent', 'status',
]
          ...
          >
            ...
            <template v-else-if="col === 'status' && canInlineEdit">
              <select
                ...
                @change="$emit('updateCell', row.id, 'status', $event.target.value)"
              >
            ...
            <template v-else>
              {{ col === 'id' ? row[col] : truncate(formatValue(row, col)) }}
            </template>
          </td>
