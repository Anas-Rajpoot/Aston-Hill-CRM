...
async function load() {
  loading.value = true
  try {
    const [notifRes, announcementRes] = await Promise.allSettled([
      api.get('/notification-logs'),
      api.get('/announcements'),
    ])
    if (notifRes.status === 'fulfilled') {
      notifications.value = notifRes.value.data?.data ?? notifRes.value.data ?? []
    }
    if (announcementRes.status === 'fulfilled') {
      announcements.value = announcementRes.value.data?.data ?? announcementRes.value.data ?? []
    }
  } catch { /* silent */ }
...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <button @click="load" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-brand-primary text-white text-sm font-medium hover:bg-brand-primary-hover transition">
    ...
    <div v-else>
      ...
      <ul v-else class="space-y-3">
        <li v-for="a in announcements" :key="a.id" class="bg-white rounded-xl border border-gray-200 p-5 hover:border-brand-primary-muted transition">
          ...
          <p class="text-sm text-gray-600 mt-1">{{ a.content || a.message || '' }}</p>
        </li>
      </ul>
    </div>
  </div>
