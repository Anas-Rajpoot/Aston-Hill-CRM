    ...
    <div v-else-if="user" class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div class="flex flex-wrap items-baseline justify-between gap-2 px-6 py-5 border-b border-gray-200">
        <div class="flex flex-wrap items-baseline gap-2">
          <h1 class="text-xl font-semibold text-gray-900">Employee Details</h1>        </div>
        <button
          type="button"
          @click="onClose"
              ...
              <div class="flex items-baseline gap-x-4 border-b border-gray-100 py-2.5">
                ...
                <dd class="min-w-0 flex flex-wrap gap-2">
                  <span
                    ...
                    :key="r.id"
                    class="inline-flex rounded-full bg-brand-primary-light px-3 py-1 text-xs font-medium text-brand-primary-hover"
                  >
          ...
          <section>
            ...
            <dl class="space-y-0 text-sm">
              <div class="flex items-baseline gap-x-4 border-b border-gray-100 py-2.5">
                ...
                <dd class="min-w-0 flex items-center gap-1.5 font-medium" :class="hasSystemAccess === 'Yes' ? 'text-brand-primary' : 'text-gray-600'"><svg v-if="hasSystemAccess === 'Yes'" class="w-5 h-5 text-brand-primary shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg> {{ hasSystemAccess }}</dd>
              </div>
              <div class="flex items-baseline gap-x-4 border-b border-gray-100 py-2.5">
                <dt class="w-[13rem] shrink-0 text-gray-500 font-medium">Has Cisco Extension:</dt>
                <dd class="min-w-0 flex items-center gap-1.5 font-medium" :class="hasCiscoExtension === 'Yes' ? 'text-brand-primary' : 'text-gray-600'"><svg v-if="hasCiscoExtension === 'Yes'" class="w-5 h-5 text-brand-primary shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg> {{ hasCiscoExtension }}</dd>
              </div>
              <div class="flex items-baseline gap-x-4 border-b border-gray-100 py-2.5">
                <dt class="w-[13rem] shrink-0 text-gray-500 font-medium">WhatsApp Assigned:</dt>
                <dd class="min-w-0 flex items-center gap-1.5 font-medium" :class="whatsAppAssigned === 'Yes' ? 'text-brand-primary' : 'text-gray-600'"><svg v-if="whatsAppAssigned === 'Yes'" class="w-5 h-5 text-brand-primary shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg> {{ whatsAppAssigned }}</dd>
              </div>
            </dl>
          </section>
        </div>
      ...
      <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-wrap items-center justify-end gap-3">
        ...
        <button
          ...
          type="button"
          @click="onEdit"
          class="inline-flex items-center gap-2 rounded-lg border border-brand-primary bg-brand-primary px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-primary-hover"
        >
