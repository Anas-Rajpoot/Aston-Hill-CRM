...
const canManage = computed(() => canModuleAction(auth.user, 'teams', 'edit', ['teams.manage_members']))
...
<template>
  <div class="space-y-6">
    ...
    <router-link :to="`/teams/${route.params.id}`" class="inline-flex items-center gap-1 text-sm text-brand-primary hover:text-brand-primary-hover font-medium">
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col" style="max-height: 70vh">
        <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
          ...
          <input v-model="memberSearch" type="text" class="w-full rounded-lg border-gray-300 px-3 py-1.5 text-sm focus:border-brand-primary focus:ring-brand-primary" placeholder="Search members…" />
        </div>
        <div class="flex-1 overflow-y-auto divide-y divide-gray-100">
          ...
          <div v-for="m in filteredMembers" :key="m.id" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
            <span class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-brand-primary-light text-xs font-bold text-brand-primary-hover flex-shrink-0">{{ initials(m.name) }}</span>
      ...
      <div v-if="canManage" class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col" style="max-height: 70vh">
        <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
          <div class="flex items-center justify-between mb-2">
            ...
            <button
              ...
              type="button"
              class="inline-flex items-center gap-1.5 rounded-lg bg-brand-primary px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-primary-hover disabled:opacity-50"
              :disabled="addLoading"
              @click="addSelected"
            >
          ...
          <div class="flex gap-2">
            <input v-model="availableSearch" type="text" class="flex-1 rounded-lg border-gray-300 px-3 py-1.5 text-sm focus:border-brand-primary focus:ring-brand-primary" placeholder="Search users…" />
            <select v-if="departments.length" v-model="availableFilter" class="rounded-lg border-gray-300 px-2 py-1.5 text-sm focus:border-brand-primary focus:ring-brand-primary">
        ...
        <div class="flex-1 overflow-y-auto divide-y divide-gray-100">
          ...
          <label
            ...
            :key="u.id"
            class="flex items-center gap-3 px-4 py-3 hover:bg-brand-primary-light cursor-pointer"
            :class="selectedToAdd.includes(u.id) ? 'bg-brand-primary-light' : ''"
          >
            <input type="checkbox" :checked="selectedToAdd.includes(u.id)" class="rounded border-gray-300 text-brand-primary focus:ring-brand-primary" @change="toggleSelect(u.id)" />
